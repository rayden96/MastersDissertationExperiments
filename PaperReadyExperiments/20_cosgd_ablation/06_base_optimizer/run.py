...
def build_cells():
    return [
        ...
        {"label": "cosgd", "method": "cosgd",
         "hp": {"cosgd_method": "modified_gs_negative", "combine": "mean"}},
    ]
...
def main():
    ...
    args = ap.parse_args()
    if args.smoke:
        args.bases = ["sgd", "adam"]; args.seeds = [2026]; args.epochs = 1; args.train_subset = 2000
    # one harness call per base so each base's baseline+cosgd land together
    for base in args.bases:
        run_cosgd_sweep(axis_name=f"20_06_base_optimizer_{base}", cells=build_cells(),
                        bases=[base], dataset=args.dataset, seeds=args.seeds,
                        epochs=args.epochs, out_root=_HERE / "results" / base,
