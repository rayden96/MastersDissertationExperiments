"""
BoGrad (scrutinised): Batch Orthogonalised Gradient wrapper for PyTorch optimisers.

Refined version of the IJCNN-era BOSGD. The core projection algorithm is the
same sequential Gram-Schmidt subtraction as the original; this file adds
correctness/ergonomic improvements that don't alter the math.
...
class BoGrad(Optimizer):
    ...
    project_stage : {"gradient", "update"}, default "gradient"
        ...
        - "update": run base_optimizer.step(), project the applied parameter
          ...
          buffer.
    projection_mode : {"full", "negative"}, default "full"
    ...
    **base_optimizer_kwargs
    ...
    def _project(self, x_flat: Tensor, buffer: List[Tensor]) -> Tuple[Tensor, Dict[str, float]]:
        ...
        if self.orth_method == "sequential":
            projected = self._project_sequential(x_flat, buffer)
        elif self.orth_method == "qr":
            projected = self._project_qr(x_flat, buffer)
        else:
    ...
    def _project_qr(self, x_flat: Tensor, buffer: List[Tensor]) -> Tensor:
        ...
        if self.projection_mode == "negative":
            ...
            # back to sequential behaviour under "negative". Users who want
            # true orthogonal projection should use "full".
